...
export default function MyPolls({ polls, loading, onDelete }) {
  ...
  const filtered = polls
    .filter(p => {
      ...
      const matchStatus = statusFilter === "all" ? true : statusFilter === "active" ? !p.isExpired : p.isExpired;
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Skeleton h={52} r={14} />
        {[...Array(4)].map((_, i) => <Skeleton key={i} h={90} r={14} />)}
      </div>
    );
  }
    ...
    <div className="mp-root">
      ...
      <div className="mp-header">
        <div>
          ...
          <div className="mp-subtitle">{polls.length} poll{polls.length !== 1 ? "s" : ""} total · {polls.filter(p => !p.isExpired).length} active</div>
        </div>
      ...
      <div className="mp-controls">
        ...
        <div className="mp-filters">
          {["all", "active", "closed"].map(s => (
            <button key={s} className={`mp-filter-btn ${statusFilter === s ? "active" : ""}`} onClick={() => setStatusFilter(s)}>
      ...
      {filtered.length === 0 ? (
        <div className="mp-empty">
          ...
          <div className="mp-empty-h">{search ? "No polls match your search" : polls.length === 0 ? "No polls yet" : "No polls in this category"}</div>
          <div className="mp-empty-p">
            {search ? "Try a different search term" : polls.length === 0 ? "Create your first poll to get started" : "Try a different filter"}
          </div>
          {polls.length === 0 && <Link to="/create" className="mp-new-btn" style={{ marginTop: 18 }}>Create Poll</Link>}
        </div>
      ) : (
        <div className="mp-list">
          {filtered.map((poll, i) => {
            ...
            return (
              <div key={poll._id} className="mp-poll-card" style={{ animationDelay: `${i * 40}ms` }}>
                <div className="mp-poll-left">
                  ...
                  <div className="mp-poll-meta">
                    ...
                    <span className="mp-meta-item">📅 Created {createdAt.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                    {!poll.isExpired && <span className="mp-meta-item" style={{ color: daysLeft <= 1 ? "#f87171" : "rgba(255,255,255,.35)" }}>
                      ⏰ {daysLeft === 0 ? "Expires today" : `${daysLeft}d left`}
                    </span>}
                  </div>
                  {votes > 0 && topOption && (
                    <div className="mp-leading">
                      Leading: <span style={{ color: "#818cf8", fontWeight: 700 }}>{topOption.text}</span> with {topOption.votes} votes
                    </div>
                  )}
                  ...
                  <div className="mp-options-preview">
                    {poll.options.slice(0, 3).map((opt, j) => {
                      ...
                      return (
                        <div key={j} className="mp-opt-row">
                          <div className="mp-opt-label">{opt.text.slice(0, 24)}{opt.text.length > 24 ? "…" : ""}</div>
                    ...
                    {poll.options.length > 3 && (
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,.28)", marginTop: 3 }}>+{poll.options.length - 3} more options</div>
                    )}
                  </div>
                </div>
                <div className="mp-poll-actions">
                  <button className="mp-act-btn view" onClick={() => navigate(`/poll/${poll.slug}`)}>
                    View Poll
                  </button>
                  <button className="mp-act-btn copy" onClick={() => {
                    navigator.clipboard.writeText(`${window.location.origin}/poll/${poll.slug}`);
                  }}>
                    Copy Link
                  </button>
                  <button className="mp-act-btn delete" onClick={() => onDelete(poll._id)}>
                    Delete
                  </button>
                </div>
              </div>
