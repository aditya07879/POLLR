...
function buildChartData(polls, filter) {
  ...
  if (filter === "daily") {
    ...
    for (let h = 0; h < 24; h += 2) {
      buckets.push({ label: `${h}:00`, votes: 0, polls: 0, start: new Date(now.getFullYear(), now.getMonth(), now.getDate(), h), end: new Date(now.getFullYear(), now.getMonth(), now.getDate(), h + 2) });
    }
...
export default function Overview({ polls, loading, filter, animStarted }) {
  ...
  const mostPopular = polls.length > 0 ? polls.reduce((best, p) => totalVotes(p) > totalVotes(best) ? p : best, polls[0]) : null;
  ...
  const statCards = [
    ...
    { label: "Active Rate", val: cEngagement + "%", icon: "⬡", accent: "#ec4899", badge: "engagement", raw: true },
  ];
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
          {[...Array(4)].map((_, i) => <Skeleton key={i} h={110} r={18} />)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <Skeleton h={230} r={18} /><Skeleton h={230} r={18} />
        </div>
      </div>
    );
  }
  ...
  if (polls.length === 0) {
    return (
      <div className="ov-empty-wrap">
        ...
        <div className="ov-empty-p">Create your first poll and watch live votes roll in. Charts, trends, and insights — all waiting for real data.</div>
        <Link to="/create" className="ov-cta-btn">✦ Create First Poll</Link>
      </div>
    );
  }
    ...
    <div className="ov-root">
      ...
      <div className="ov-stats-grid">
        {statCards.map((s, i) => (
          <div key={s.label} className="ov-stat-card" style={{ animationDelay: `${i * 60}ms` }}>
            <div className="ov-stat-ico" style={{ background: `${s.accent}22`, color: s.accent }}>{s.icon}</div>
            <div className="ov-stat-num">{s.raw ? s.val : s.val}</div>
            <div className="ov-stat-lbl">{s.label}</div>
            <div className="ov-stat-badge" style={{ color: s.accent, background: `${s.accent}14` }}>{s.badge}</div>
          </div>
        ))}
      </div>
          ...
          {hasChartData ? (
            <ResponsiveContainer width="100%" height={190}>
              <AreaChart data={chartData} margin={{ top: 0, right: 0, left: -28, bottom: 0 }}>
                ...
                <XAxis dataKey="day" tick={{ fill: chartTickColor, fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: chartTickColor, fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip content={<ChartTooltip />} />
                <Area type="monotone" dataKey="votes" name="votes" stroke="#6366f1" strokeWidth={2.5} fill="url(#aGrad)" dot={false} activeDot={{ r: 5, fill: "#6366f1", stroke: "#fff", strokeWidth: 2 }} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <EmptyChart message="No voting activity this period" sub="Votes will appear here once users participate" />
          )}
        </div>
          ...
          {hasChartData ? (
            <ResponsiveContainer width="100%" height={190}>
              <BarChart data={chartData} margin={{ top: 0, right: 0, left: -28, bottom: 0 }}>
                ...
                <XAxis dataKey="day" tick={{ fill: chartTickColor, fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: chartTickColor, fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="polls" name="polls" fill="url(#bGrad)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <EmptyChart message="No polls created this period" sub={`Switch to 'Monthly' to see older data`} />
          )}
        </div>
      </div>
      ...
      <div className="ov-activity-row">
        <div className="ov-card">
          ...
          {recentActivity.length > 0 ? recentActivity.map((poll) => (
            <div key={poll._id} className="ov-act-item">
              <div className="ov-act-dot" style={{ background: poll.isExpired ? "#6366f1" : "#10b981" }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="ov-act-q">{poll.question.length > 40 ? poll.question.slice(0, 40) + "…" : poll.question}</div>
                <div className="ov-act-meta">
                  {new Date(poll.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })} ·{" "}
                  {totalVotes(poll)} votes ·{" "}
                  <span style={{ color: poll.isExpired ? "#f87171" : "#10b981" }}>{poll.isExpired ? "closed" : "active"}</span>
                </div>
              </div>
            </div>
          )) : (
        ...
        <div className="ov-card">
          ...
          {topOptions.length > 0 ? topOptions.map((item, i) => {
            ...
            return (
              <div key={i} className="ov-lb-row">
                <div className="ov-lb-rank" style={{ color: rankColors[i] }}>#{i + 1}</div>
                <div className="ov-lb-text" title={item.text}>{item.text}</div>
                <div className="ov-lb-bar-bg">
                  <div className="ov-lb-bar-fill" style={{ width: animStarted ? pct + "%" : "0%", background: `linear-gradient(90deg,${rankColors[i]},${COLORS[(i + 1) % COLORS.length]})` }} />
                </div>
