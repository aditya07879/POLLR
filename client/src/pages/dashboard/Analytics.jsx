...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: tooltipBg, border: "1px solid rgba(99,102,241,.3)", borderRadius: 12, padding: "10px 16px", backdropFilter: "blur(20px)", boxShadow: "0 4px 20px rgba(0,0,0,.15)" }}>
      <div style={{ fontSize: 11, color: tooltipLabelColor, marginBottom: 4 }}>{label}</div>
      {payload.map((p, i) => (
        <div key={i} style={{ fontSize: 13, fontWeight: 700, color: p.color || "#6366f1" }}>{p.value} {p.name}</div>
      ))}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
      <div style={{ position: "relative", width: size, height: size }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, color: pctColor }}>
          {pct}%
        </div>
      </div>
      {label && <div style={{ fontSize: 12, fontWeight: 700, color: labelColor, textAlign: "center" }}>{label}</div>}
      {sublabel && <div style={{ fontSize: 11, color: sublabelColor, textAlign: "center" }}>{sublabel}</div>}
    </div>
  );
}
...
export default function Analytics({ polls, loading, filter }) {
  ...
  // Fill rate: how many polls have at least 1 vote
  ...
  // Completion: closed polls / total polls
  const completionRate = polls.length > 0 ? Math.round((closedPolls.length / polls.length) * 100) : 0;

  // Pie data: top polls by votes
  const pieData = polls
  ...
  // Options comparison for selected poll
  const focusPoll = selectedPoll ? polls.find(p => p._id === selectedPoll) : (polls.length > 0 ? polls.reduce((best, p) => totalVotes(p) > totalVotes(best) ? p : best, polls[0]) : null);
  const focusVotes = focusPoll ? totalVotes(focusPoll) : 0;
  const focusBarData = focusPoll ? focusPoll.options.map(o => ({
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
          {[...Array(3)].map((_, i) => <Skeleton key={i} h={160} r={18} />)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <Skeleton h={240} r={18} /><Skeleton h={240} r={18} />
        </div>
      </div>
    );
  }
  ...
  if (polls.length === 0) {
    return (
      <div className="an-empty-wrap">
        ...
        <div className="an-empty-p">Create polls, share them, and collect votes. All your participation data will show up here.</div>
        <a href="/create" className="an-cta-btn">Create Your First Poll</a>
      </div>
    );
  }
  ...
  return (
    <div className="an-root">
      ...
      <div className="an-period-banner">
        ...
        Showing analytics for the <strong>{filter === "daily" ? "last 24 hours" : filter === "weekly" ? "last 7 days" : "last 30 days"}</strong>
        &nbsp;· {filteredPolls.length} poll{filteredPolls.length !== 1 ? "s" : ""} · {filteredVotes} votes
      </div>
      ...
      <div className="an-card" style={{ marginBottom: 16 }}>
        ...
        <div className="an-rings-row">
          <div className="an-ring-item">
            <ProgressRing pct={activeRatio} color="#6366f1" label="Active Ratio" sublabel={`${activePolls.length} of ${polls.length} polls active`} />
          </div>
          <div className="an-ring-divider" />
          <div className="an-ring-item">
            <ProgressRing pct={fillRate} color="#06b6d4" label="Fill Rate" sublabel={`${pollsWithVotes} polls received votes`} />
          </div>
          <div className="an-ring-divider" />
          <div className="an-ring-item">
            <ProgressRing pct={completionRate} color="#10b981" label="Completion" sublabel={`${closedPolls.length} polls completed`} />
          </div>
          <div className="an-ring-divider" />
          <div className="an-ring-item">
            <ProgressRing
              pct={polls.length > 0 ? Math.min(100, Math.round((allVotes / (polls.length * 10)) * 100)) : 0}
              color="#ec4899"
              label="Vote Density"
        ...
        {polls.length === 0 && (
          <div className="an-empty-rings">Not enough data yet — share your polls to collect votes</div>
        )}
      </div>

      {/* Distribution + Per-poll breakdown */}
      <div className="an-charts-2col">
        ...
        <div className="an-card">
          ...
          {pieData.length > 0 ? (
            ...
            <div className="an-empty-chart">
              ...
              <div style={{ fontSize: 11, color: emptySubColor }}>Share your polls to start collecting votes</div>
            </div>
          )}
        </div>
        ...
        <div className="an-card">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            ...
            {polls.length > 1 && (
              <select className="an-poll-select" value={selectedPoll || focusPoll?._id || ""} onChange={e => setSelectedPoll(e.target.value)}>
                {polls.map(p => (
                  <option key={p._id} value={p._id}>{p.question.slice(0, 32)}{p.question.length > 32 ? "…" : ""}</option>
                ))}
              </select>
            )}
          </div>
          {focusBarData.length > 0 && focusVotes > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={focusBarData} layout="vertical" margin={{ top: 0, right: 40, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={chartGridColor} horizontal={false} />
                <XAxis type="number" tick={{ fill: chartTickColor, fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis type="category" dataKey="name" tick={{ fill: chartTickColorStrong, fontSize: 11 }} axisLine={false} tickLine={false} width={80} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="votes" name="votes" fill="#6366f1" radius={[0, 6, 6, 0]}>
                  {focusBarData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : (
          ...
          {polls.map(poll => {
            ...
            return (
              <div key={poll._id} className="an-table-row">
                <div className="an-td an-td-q">{poll.question.length > 36 ? poll.question.slice(0, 36) + "…" : poll.question}</div>
                <div className="an-td center">
                  <span className={`an-status-pill ${poll.isExpired ? "closed" : "active"}`}>{poll.isExpired ? "Closed" : "Active"}</span>
                </div>
                ...
                <div className="an-td center" style={{ color: "#818cf8", fontSize: 12 }}>
                  {top && votes > 0 ? `${top.text.slice(0, 16)} (${Math.round((top.votes / votes) * 100)}%)` : "—"}
                </div>
                <div className="an-td center">{poll.options.length > 0 ? Math.round(votes / poll.options.length) : 0}</div>
              </div>
            );
          })}
        </div>
