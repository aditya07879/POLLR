...
function InsightCard({ icon, color, title, value, sub, badge }) {
  return (
    <div className="ins-card" style={{ borderColor: `${color}30`, background: `${color}0a` }}>
      <div className="ins-card-ico" style={{ background: `${color}1a`, color }}>
        {icon}
      </div>
      <div className="ins-card-title" style={{ color: `${color}cc` }}>
        {title}
      </div>
      <div className="ins-card-val">{value}</div>
      {sub && <div className="ins-card-sub">{sub}</div>}
      {badge && (
        <div className="ins-card-badge" style={{ background: `${color}18`, color }}>
          {badge}
        </div>
      )}
    </div>
...
export default function Insights({ polls, loading }) {
  ...
  const insights = useMemo(() => {
    ...
    // Most popular poll
    const mostPopular = polls.reduce(
      (best, p) => (totalVotes(p) > totalVotes(best) ? p : best),
      polls[0]
    );
    ...
    const mostDecisive = polls
      ...
      .map((p) => {
        ...
        const margin =
          sorted.length > 1 ? Math.round(((sorted[0].votes - sorted[1].votes) / votes) * 100) : 100;
    ...
    const healthScore =
      polls.length > 0
        ? Math.round(
            ((pollsWithVotes / polls.length) * 0.5 +
              (activePolls / polls.length) * 0.3 +
              Math.min(allVotes / (polls.length * 10), 1) * 0.2) *
              100
          )
        : 0;
  ...
  if (polls.length === 0) {
    return (
      <div className="ins-empty-wrap">
        ...
        <div className="ins-empty-p">
          Once you create polls and collect votes, you'll see winner predictions, poll health
          scores, engagement trends, and smart summaries here.
        </div>
        <Link to="/create" className="ins-cta-btn">
    ...
    <div className="ins-root">
      ...
      <div className="ins-health-banner">
        <div className="ins-health-left">
          ...
          <div className="ins-health-val">
            {insights?.healthScore ?? 0}
            <span>/100</span>
          </div>
          <div className="ins-health-desc">
            {insights?.healthScore >= 80
              ? "🟢 Excellent engagement — your polls are thriving!"
              : insights?.healthScore >= 50
                ? "🟡 Good activity — keep sharing your polls for more data"
        ...
        <div className="ins-health-bars">
          {[
            {
              label: "Vote Coverage",
              val: insights ? Math.round((insights.pollsWithVotes / polls.length) * 100) : 0,
              color: "#6366f1",
            },
            {
              label: "Active Ratio",
              val: insights ? Math.round((insights.activePolls / polls.length) * 100) : 0,
              color: "#10b981",
            },
            {
              label: "Vote Density",
              val: insights ? Math.min(100, Math.round((allVotes / (polls.length * 10)) * 100)) : 0,
              color: "#ec4899",
            },
          ].map((item) => (
            <div key={item.label} className="ins-health-bar-row">
              ...
              <div className="ins-health-bar-track">
                <div
                  className="ins-health-bar-fill"
                  style={{ width: item.val + "%", background: item.color }}
                />
              </div>
              <div className="ins-health-bar-val" style={{ color: item.color }}>
                {item.val}%
              </div>
            </div>
          ))}
      ...
      <div className="ins-grid">
        <InsightCard
          ...
          value={
            insights?.mostActive
              ? insights.mostActive.question.slice(0, 40) +
                (insights.mostActive.question.length > 40 ? "…" : "")
              : "No data"
          }
        ...
        <InsightCard
          ...
          value={
            insights?.mostDecisive ? `${insights.mostDecisive.margin}% lead` : "Not enough data"
          }
          sub={
            insights?.mostDecisive
              ? insights.mostDecisive.poll.question.slice(0, 36) + "…"
              : "Need 2+ options with votes"
          }
          badge="Clear winner"
        />
        <InsightCard
          ...
          sub={
            insights?.closestRace
              ? `Only ${insights.closestRace.margin} vote${insights.closestRace.margin !== 1 ? "s" : ""} apart`
              : "Need more votes"
          }
          badge="Neck and neck"
        />
        ...
        <InsightCard
          ...
          sub={
            polls.length > 0
              ? `${Math.round(((insights?.pollsWithVotes ?? 0) / polls.length) * 100)}% fill rate`
              : ""
          }
          badge="Coverage"
        />
        ...
        <div className="ins-recs-list">
          {allVotes === 0 && (
            <div className="ins-rec-item">
              ...
              <div>
                ...
                <div className="ins-rec-p">
                  Your polls exist but have no votes yet. Copy the poll link and share it on social
                  media or with your community.
                </div>
              </div>
            </div>
          )}
          {polls.filter((p) => !p.isExpired).length === 0 && (
            <div className="ins-rec-item">
              ...
              <div>
                ...
                <div className="ins-rec-p">
                  You have no active polls. Create a new poll to start collecting fresh data.
                </div>
              </div>
            </div>
          )}
          {polls.filter((p) => !p.isExpired).length > 0 && allVotes > 0 && (
            <div className="ins-rec-item">
              ...
              <div>
                ...
                <div className="ins-rec-p">
                  Keep momentum going — consider creating follow-up polls or sharing existing ones
          ...
          {polls.length < 3 && (
            <div className="ins-rec-item">
              ...
              <div>
                ...
                <div className="ins-rec-p">
                  With 5+ polls, you'll see trend analysis, engagement patterns, and audience
                  preference maps.
                </div>
              </div>
            </div>
          ...
          {insights?.closestRace && (
            <div className="ins-rec-item">
              ...
              <div>
                <div className="ins-rec-h">
                  "{insights.closestRace.poll.question.slice(0, 40)}…" is a very close race
                </div>
                <div className="ins-rec-p">
                  Top options are only {insights.closestRace.margin} vote(s) apart. Share this poll
                  to break the tie!
                </div>
              </div>
            </div>
